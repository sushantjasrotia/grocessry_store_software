...
var productUpdateApiUrl = 'http://127.0.0.1:5000/insertOrder';
...
function calculateValue() {
    var total = 0;
    $(".product-item").each(function(index) {
        var qtyInput = $(this).find('.product-qty');
        var priceInput = $(this).find('.product-price');
        var qty = parseFloat(qtyInput.val());
        var price = parseFloat(priceInput.val());

        // Event listener for price change
        priceInput.on('input', function() {
            qtyInput.val(0); // Set quantity to 0 when price changes
        });
